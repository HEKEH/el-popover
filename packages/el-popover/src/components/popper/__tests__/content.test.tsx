...
function mountContent(props = {}) {
  return mount(<ElContent {...props}>{AXIOM}</ElContent>, {
    global: {
      provide: {
...
function mountWrappedContent(props = {}) {
...
describe('<ElPopperContent />', () => {
  describe('with triggerRef provided', () => {
    ...
    it('should mount the component correctly and set popperInstance correctly', async () => {
      ...
      expect(wrapper.classes()).toEqual(['ep-popper', 'is-dark']);
    ...
    it('should be able to be pure and themed', async () => {
      wrapper = mountContent() as VueWrapper<any>;
      await nextTick();
      ...
      expect(wrapper.classes()).toEqual(['ep-popper', 'is-pure', 'is-custom']);
    });
    ...
    describe('instantiate popper instance', () => {
      ...
      it('should be able to update the reference node', async () => {
        ...
        const contentRef = w.vm.contentRef as { popperInstanceRef: any };
